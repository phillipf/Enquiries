...
CallQual2 <- CallQual 
             #select(USES,
                    #MEMO_POS1:MEMO_POS10) 

#CallQual2[is.na(CallQual2)] <- ""

attributes(CallQual2)$problems <- ""

#CallQual2 <- CallQual2 %>%
             #mutate(MEMO = paste(MEMO_POS1, 
                                 #MEMO_POS2, 
                                 #MEMO_POS3, 
                                 # MEMO_POS4,
                                 # MEMO_POS5,
...
#test if any columns contain a table
    ...
    test1 <- apply(probs, 2, function(x) sapply(x, test1fun))

    test1a <- unlist(apply(test1, 2, which))
    id <- unname(unlist(apply(test1, 2, which)))
    column <- sapply(id, function(x) which(unlist(apply(test1, 2, which)) == x))

for(i in 1:nrow(probs)) {
    y = probs[i,] 
    a = which(grepl("Enquiry.*", y))[1]
    b = which(y == "RES" | y == "NONRES")
    c = b - a
    if(length(c) != 0) {
      if(c > 1) {
           d = as.character(as.vector(y[,seq(a, b-1)]))
           e = d[!is.na(d)]
           probs[i,a] <- paste(y[[1,a]], paste(e, sep="", collapse=""))
           probs[i, b-1] <- NA
           
           while(a + c <= ncol(probs)) {
                 probs[i, a + 1] <- y[[1, a + c]]
                 probs[i, a + c] <- NA
                 a = a + 1
                 }
           ...
           d = as.character(as.vector(y[,seq(a, ncol(probs))]))
           e = d[!is.na(d)]
           probs[i,a] <- paste(y[[1,a]], paste(e, sep="", collapse=""))
           probs[i, ncol(probs)] <- NA                                
    } 
}

lapply(1:6, function(i) {y = probs[i,] 
                                    a = which(grepl("Enquiry.*", y))[1]
                                    b = which(y == "RES" | y == "NONRES")
                                    c = b - a
                                    if(length(c) != 0) {
...
MemoNotes <- Enquiries::Memo(MEMO = CallQual2$MEMO, USES = CallQual$USES)

MemoNotes[] <- lapply(MemoNotes, as.character)

Enquiry <- function(x) {
  
  if(grepl("Enquiry\\s=\\s[[:alpha:]]+", x) == TRUE) {
    
    return(gsub("(Enquiry\\s=\\s)([[:alpha:]]+)", "\\2", x))
  }
...
Result <- function(x) {
    ...
    if(grepl("Result\\s*=\\s*", x) == TRUE) {
      #Result = #
      a <- gsub("(Result\\s*=\\s*)(.*)", "\\2", x)
      
          if(lapply(a, function(x) grepl("(.*)-(.*)-(.*)", x) == TRUE)) {
            b <- gsub("(.*)-(.*)-(.*)", "\\1", a)
            c <- gsub("(.*)-(.*)-(.*)", "\\2", a)
            d <- gsub("(.*)-(.*)-(.*)", "\\3", a)
            return(data.frame(MEC.1 = b, MEC.2 = c, MEC.3 = d))
          }
          
          else if(lapply(a, function(x) grepl("(.*)-(.*)", a) == TRUE)) {
            
            b <- gsub("(.*)-(.*)", "\\1", a)
            c <- gsub("(.*)-(.*)", "\\2", a)
            #d <- NA
            return(data.frame(MEC.1 = b, MEC.3 = c))
          }
      
          else if(lapply(a, function(x) grepl("(.*)", a) == TRUE)) {

            return(data.frame(MEC.1 = a))
          }
