...
InternalSRM_ConType <- ddply(CallQual3 %>% filter(SRM == "SRM", USES == "NONRES", ENQUIRY == "INTERNAL SRM"), .(BEGIN, END), summarise, Total = length(MEMO_CONSUMER)) %>%
                      mutate(END = paste(END, "_end", sep = "")) %>%
                      arrange(-Total) 


Filter <- ddply(CallQual3 %>% filter(SRM == "SRM", USES == "NONRES", ENQUIRY == "INTERNAL SRM"), .(BEGIN, END), summarise, Total = length(MEMO_CONSUMER)) %>%
                      arrange(-Total) %>%
                      mutate(cumulative = cumsum(Total)/sum(Total)) %>%
                      filter(cumulative < 0.93) 

InternalSRM_ConType[!(InternalSRM_ConType$BEGIN %in% Filter$BEGIN), ]$BEGIN <- "OTHER"


EndUse_enquiry <- aggregate(Total ~ BEGIN + END, InternalSRM_ConType, sum) %>%
                  mutate(Total = Total/sum(Total))

colnames(EndUse_enquiry) <- c("source", "target", "value")

sankeyPlot(EndUse_enquiry, name = "InternalSRM")

htmltools::includeHTML("C:/Enquiries/InternalSRM.html")
...
```{r Metering SRM, echo=FALSE}

InternalSRM_ConType <- ddply(CallQual3 %>% filter(SRM == "SRM", USES == "NONRES", ENQUIRY == "METERING"), .(BEGIN, END), summarise, Total = length(MEMO_CONSUMER)) %>%
                      mutate(END = paste(END, "_end")) %>%
                      arrange(-Total) 


Filter <- ddply(CallQual3 %>% filter(SRM == "SRM", USES == "NONRES", ENQUIRY == "METERING"), .(BEGIN, END), summarise, Total = length(MEMO_CONSUMER)) %>%
                      arrange(-Total) %>%
                      mutate(cumulative = cumsum(Total)/sum(Total)) %>%
                      filter(cumulative < 0.93) 
...
sankeyPlot(EndUse_enquiry, name = "MeteringSRM")

htmltools::includeHTML("C:/Enquiries/MeteringSRM.html")
...
```{r TRADEWASTE SRM, echo=FALSE}

InternalSRM_ConType <- ddply(CallQual3 %>% filter(SRM == "SRM", USES == "NONRES", ENQUIRY == "TRADE WASTE"), .(BEGIN, END), summarise, Total = length(MEMO_CONSUMER)) %>%
                      mutate(END = paste(END, "_end")) %>%
                      arrange(-Total) 


Filter <- ddply(CallQual3 %>% filter(SRM == "SRM", USES == "NONRES", ENQUIRY == "TRADE WASTE"), .(BEGIN, END), summarise, Total = length(MEMO_CONSUMER)) %>%
                      arrange(-Total) %>%
                      mutate(cumulative = cumsum(Total)/sum(Total)) %>%
                      filter(cumulative < 0.93) 
...
sankeyPlot(EndUse_enquiry, name = "TradewasteSRM")

htmltools::includeHTML("C:/Enquiries/TradewasteSRM.html")
