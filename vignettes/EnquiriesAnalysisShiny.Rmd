...
inputPanel(
  selectInput("FYConType", label = "Financial year:", choices = c("All", unique(InternalSRM_ConType$FY)), selected = 2016),
  selectInput("XConType", label = "xaxis:", choices = unique(InternalSRM_ConType$XAXIS, "NONE"), selected = "NONE")
)
...
MeteringSRM_Action <- ddply(CallQual3 %>% filter(SRM == "SRM", USES == "NONRES", ENQUIRY == "METERING"), .(FY, QUARTER, MONTH, ACC_TYPE, ACTION), summarise, Total = length(MEMO_CONSUMER)) %>%
                      arrange(-Total) #%>%
                      #mutate(cumulative = cumsum(Total)/sum(Total))# %>%
                      # filter(cumulative < 0.93)
...
MeteringSRM_Action <- aggregate(Total ~ FY + QUARTER + MONTH + ACC_TYPE + ACTION, MeteringSRM_Action, sum)
