...
SRM_enquiry_RES <- SRM_enquiry %>%
                   ...
                   select(ENQUIRY, Total, CumulativePercent, Percent)
...
print(xtable(SRM_enquiry_NRES %>% select(-CumulativePercent), align = "cccr",latex.environments="center", format.args = list(digits = 2, format = c("d","d","s","f"))), type = "html", include.rownames=FALSE)
...
print(xtable(SRM_enquiry_RES %>% select(-CumulativePercent), align = "cccr",latex.environments="center", format.args = list(digits = 2, format = c("d","d","s","f"))), type = "html", include.rownames=FALSE)
...
shinyApp(
  ui = showOutput('sankey', 'C:/rCharts_d3_sankey/libraries/widgets/d3_sankey'),
...
TradeWasteSRM_Action <- ddply(CallQual3 %>% filter(SRM == "SRM", USES == "NONRES", ENQUIRY == "TRADE WASTE"), .(FY, QUARTER, MONTH, ACC_TYPE, ACTION), summarise, Total = length(MEMO_CONSUMER)) %>%
                      arrange(-Total) #%>%
                      #mutate(cumulative = cumsum(Total)/sum(Total))# %>%
                      # filter(cumulative < 0.93)
...
TradeWasteSRM_Action <- aggregate(Total ~ FY + QUARTER + MONTH + ACC_TYPE + ACTION, TradeWasteSRM_Action, sum)
